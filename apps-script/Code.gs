...
function doGet(event) {
  ...
  try {
    ...
    var merged = mergeInterviewData_(false);
    var publicRows = merged.filter(function(row) {
      var status = String(row.Status || "Scheduled").toLowerCase();
      return status !== "completed" && status !== "cancelled";
    }).map(sanitizePublicRow_);
    var response = publicRows;
...
function doPost(event) {
  try {
    ...
    if (request.action === "listAdminInterviews") {
      return jsonOutput_({ ok: true, data: mergeInterviewData_(true), adminEmail: adminEmail });
    }
...
function verifyAdmin_(credential) {
  ...
  var allowed = String(properties.getProperty("ADMIN_EMAILS") || "").toLowerCase().split(",").map(function(value) { return value.trim(); }).filter(String);
  ...
  if (token.aud !== clientId || String(token.email_verified) !== "true") throw new Error("Google identity verification failed.");
...
function validateStatusRequest_(request) {
  ...
  if ((status === "Cancelled" || status === "Other") && !remarks) throw new Error(status + " requires a reason or remark.");
  if (remarks.length > 500) throw new Error("Remarks must be 500 characters or fewer.");
  if (status === "Rescheduled") {
    if (!request.rescheduledDate || !request.rescheduledFrom || !request.rescheduledTo) throw new Error("New date, start time and end time are required.");
    if (minutes_(request.rescheduledTo) <= minutes_(request.rescheduledFrom)) throw new Error("End time must be after start time.");
  }
}
...
function mergeInterviewData_(includePrivate) {
  var spreadsheet = getSpreadsheet_();
  ...
  updates.forEach(function(update) {
    ...
    if (!latest[key] || updatedAt >= latest[key].updatedAt) latest[key] = { updatedAt: updatedAt, update: update };
  });
...
function rowsAsObjects_(sheet) {
  // Keep native Date values so JSON serializes them as unambiguous ISO timestamps.
  // Display-formatted dates such as 17/8/2026 are parsed differently by browsers.
...
function makeInterviewKey_(row) {
  return [
    row["Sk Tech Register ID"], row["Interview Date"],
    row["Interview Time (From)  or  If Time Not confirmed plz select 00:00 like Assessment"],
    row["Interview Time (To) or  If Time Not confirmed plz select 00:00 like Assessment"], row.Batch
  ].map(function(value) { return String(value || "").trim().toLowerCase().replace(/\s+/g, " "); }).join("|");
}
