...
1. Create a separate standalone Apps Script project for interview status management.
2. Replace its default code with `StandaloneInterviewStatus.gs`. Keep the existing registration/email Apps Script project unchanged. The standalone deployment serves sanitized interview JSON from its root `/exec` URL.
