...
### 2. Interview Dashboard (`interviews.html` + `dashboard.css` + `dashboard.js` + `jobs.css` + `jobs.js`)
- Tab-based filtering: **All Upcoming**, **Today**, **Tomorrow**, **Available Slots**, **Job Openings**
...
- **Job Openings** tab loads jobs dynamically from a published Google Sheet CSV
- Each job card shows: icon, title, company, experience, location, employment type, description, and skills
- **View Details** modal with full job information
- **Apply Now** button follows sheet logic: Apply Link → external link; empty link + email → mailto; neither → "Contact HR"
- **WhatsApp** share button with pre-filled job details
- **Instagram** button generates a caption + square preview for manual posting
...
| `jobs.css` | Styles for job openings cards and modals |
| `jobs.js` | Logic for fetching, parsing, and displaying job openings |
...
> Note: The server must stay running. If you close PowerShell, the site stops. The script automatically uses the folder it is located in as the web root.
...
Job Openings are loaded dynamically from the Google Sheet CSV defined in `jobs.js`:

```javascript
const JOBS_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-.../pub?output=csv";
```
...
3. Set the `Status` column to **Active** so it appears on the site.
4. Set `Status` to anything else (e.g., `Inactive`, `Closed`) to hide it.

### Apply button logic

The `Apply Now` button follows this priority:

1. If `Apply Link (Optional)` is filled → open the link in a new tab.
2. Else if `HR Email (Optional)` is filled → open the default email client (`mailto:`).
3. Else → show a "Contact HR" alert.

### WhatsApp share

The **WhatsApp** button on each card opens WhatsApp Web with a pre-filled message containing the job title, company, location, experience, salary, skills, and apply link. It can also append the WhatsApp group link if provided in the sheet.

> **Note:** This is a share button, not automatic posting. To auto-post to a WhatsApp group, you need the WhatsApp Business API or a third-party service like Twilio / Make.com.

### Instagram post

The **Instagram** button opens a modal with:
- A square preview card (gradient background + job details)
- A ready-to-copy caption with hashtags

> **Note:** Instagram does not allow direct posting from a browser. Copy the generated caption and image, then paste it into the Instagram app or Meta Business Suite.
